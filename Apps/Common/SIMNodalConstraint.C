...
public:
  ...
  //! \param comp Component to constrain
  //! \param basis Basis to constrain face for
  //! \param idx Global node to constrain face to.
  ...
  //! \brief Constrain the whole patch to a given node.
...
protected:
  //! \brief Constrain a given node to another node.
  //! \param item Local index of node to constrain
  //! \param comp Component to constrain
  //! \param idx Global index of the node to constrain \a node to
...
public:
  ...
  NodalConstraintASMs1DHelper(ASMs1D* pch) : NodalConstraintASMHelper(pch) {}
...
public:
  //! \brief The constructor forwards to the parent class constructor.
  NodalConstraintASMs2DHelper(ASMs2D* pch) : NodalConstraintASMHelper(pch) {}
    ...
    switch (item) {
      ...
      case 1: // Left edge (negative I-direction)
        for (int i2 = 1; i2 <= n2; i2++, node += n1)
          this->constrainNode(ofs+node,comp,idx);
        break;
...
public:
  //! \brief The constructor forwards to the parent class constructor.
  NodalConstraintASMs3DHelper(ASMs3D* pch) : NodalConstraintASMHelper(pch) {}
  ...
  virtual void constrainEdge(int item, int comp, int basis, int idx)
  {
    int node = this->getStartNode(basis)+1;
