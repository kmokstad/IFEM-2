...
LinearFunc::LinearFunc (const char* file, int c, Real s) : scale(s)
{
  ...
  if (!is)
  {
    std::cerr <<" *** LinearFunc: Failed to open file "<< file
  ...
  while (is.good() && is.getline(temp,1024))
  {
    ...
    {
      std::cerr <<" *** LinearFunc: x-values must be monotonically increasing";
...
LinVecFunc::LinVecFunc (const char* file, int c)
{
  ...
  if (!is)
  {
    std::cerr <<" *** LinVecFunc: Failed to open file "<< file
              <<", function will be identically zero."<< std::endl;
    return;
  }
  else if (c < 2)
  {
    std::cerr <<" *** LinVecFunc: Column index ("<< c <<") should be > 1"
              <<", function will be identically zero."<< std::endl;
    return;
  }
  ...
  while (is.good() && is.getline(temp,1024))
  {
    ...
    {
      std::cerr <<" *** LinVecFunc: x-values must be monotonically increasing";
      for (size_t i = 0; i < fvals.size(); i++)
	if (i+5 == fvals.size())
          std::cerr <<"\n           :";
        else if (i+5 > fvals.size())
