...
bool ASMs2D::getParameterDomain (std::vector<RealArray>& u,
                                 std::vector<int>& corners) const
{
  ...
  corners.resize(4);
  corners[0] = 1;
  corners[1] = surf->numCoefs_u();
  corners[2] = surf->numCoefs_u()*(surf->numCoefs_v()-1) + 1;
  corners[3] = surf->numCoefs_u()*surf->numCoefs_v();
