			...
			const TimeDomain& time)
{
  ...
  for (int d = 0; d < 2; d++)
    if (-1-d == edgeDir)
      gpar[d] = { d == 0 ? surf->startparam_u() : surf->startparam_v() };
    else if (1+d == edgeDir)
      gpar[d] = { d == 0 ? surf->endparam_u() : surf->endparam_v() };
    else
